...
Whenever OpenSpec (`@fission-ai/openspec`) commands or custom slash commands (`/spec`, `/plan`, `/tasks`, `/implement`, `/review`, `/pr`) interact with change specifications, you MUST ALWAYS obey the three-pattern lifecycle structure (`changes/<AB-xxxx-name>/` for active changes, `archive/<AB-xxxx-name>/` for top-level separated archived historical snapshots created at `/pr` time, and `specs/<domain>/spec.md` for living canonical specs) and use a specific, descriptive, human-readable kebab-case name appended to the ticket ID (e.g., `AB-1001-project-setup-monorepo`). NEVER create bare folders (`AB-1001/`).
