...
- **Case A: All `✅ PASSED` (`100% compliant`)**:
  Output: `"Review complete: All checks passed (100% FRS/SDS compliance). Change remains in openspec/changes/$ARGUMENTS until /pr is run. Approved for /pr $ARGUMENTS."`
- **Case B: Any `❌ MISSING` or `⚠️ DRIFTED` findings**:
  Log all exact corrective actions and gaps directly inside `openspec/changes/$ARGUMENTS/review-log.md`. Output: `"Review failed: Gaps logged in review-log.md. Run /implement $ARGUMENTS to resolve items listed in review-log.md."` Do NOT allow `/pr`.
