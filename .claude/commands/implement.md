...
4. Ensure `.openspec/changes/$ARGUMENTS/review-log.md` exists (create if not, or ensure `openspec/changes/$ARGUMENTS/review-log.md` exists).
5. Ensure `.openspec/changes/$ARGUMENTS/fix-bundles.md` exists (create if not, or ensure `openspec/changes/$ARGUMENTS/fix-bundles.md` exists).
6. **CRITICAL NOTEAPP RULES (`[AGENTS.md, SDS §1-§3, Rules 11-20]`)**:
...
7. **GRAPH ORIENTATION (Mandatory once per ticket, before first task)**:
...
*(Run strictly at Phase Checkpoints or when a complete domain slice (`Repository -> Service -> Controller -> Router`) is finished so `test-writer` does not fail on incomplete Phase 1 schema setup)*
...
- **Case B: Tester failures OR `[WARN]/[FAIL]` (`❌ MISSING` / `⚠️ DRIFTED`) findings**:
  ...
  * If the code violated `SDS.md / FRS.md`, construct a **FIX BUNDLE** inside `openspec/changes/$ARGUMENTS/fix-bundles.md` documenting:
  ...
  * **Present the Fix Bundle to the user with ONE `[y/n]` approval request before applying the fix.** Once approved, apply the fix and re-run Step 2 & Step 3.
...
6. **STOP and report to user**:
   > "Implementation complete (`100% DoD compliant`). Run `openspec archive $ARGUMENTS` then `/pr $ARGUMENTS`."
