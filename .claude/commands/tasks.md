...
*(Execute each unchecked `[ ]` item via the `/implement` Main Claude -> Tester -> Reviewer -> Triage loop)*
...
- [ ] Confirm `review-log.md` reports all `✅ PASSED` and `fix-bundles.md` is resolved before running `openspec archive $ARGUMENTS`.
```
