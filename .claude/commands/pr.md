Prepare PR, execute proposal archive (`openspec archive`), and commit for: $ARGUMENTS
...
1. **Active Change Verification**: Verify that the proposal folder currently resides cleanly inside `openspec/changes/$ARGUMENTS/`. (Do **NOT** allow `/pr` if the proposal has already been manually moved or archived prematurely).
...
### STEP 1 — Execute Proposal Archive (`openspec archive`)

**Crucial Workflow Note (`[Rule 18]`)**: Archival happens **ONLY HERE (`/pr time`)**.
Execute the OpenSpec archive command to merge delta specifications into canonical system specs and migrate the proposal folder to the archive pattern:

```bash
npx openspec archive $ARGUMENTS -y
```

_(Note: This command merges delta scenarios from `openspec/changes/$ARGUMENTS/specs/<domain>/spec.md` into `openspec/specs/<domain>/spec.md` and moves the proposal folder out of `openspec/changes/$ARGUMENTS/`)._
...
_(This ensures `openspec/changes/` remains strictly for active proposals, and `openspec/archive/$ARGUMENTS/` is a separate top-level folder stripped of any random numbers and given its clean, meaningful ticket proposal name)._
...
2. **Strict Git Permission Gate (`CLAUDE.md §2`)**:
   - Ask: `"Run git add . (staging implementation code + separated archive proposal in openspec/archive/ + merged canonical specs)? [y/n]"` -> wait for explicit user `y`.

### STEP 3 — Conventional Commit Execution (`AGENTS.md §6`)

1. Read the archived proposal summary (`openspec/archive/$ARGUMENTS/proposal.md`).
...
1. Generate structured PR markdown:
   ...
   - [x] Proposal archived via `openspec archive $ARGUMENTS` and separated into top-level archive (`[Rule 18]`)
   - `openspec/archive/$ARGUMENTS/proposal.md` (Separated Archived Proposal Audit Trail)
