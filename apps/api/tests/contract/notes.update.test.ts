...
describe("[FRS-2.1.3] PATCH /api/v1/notes/:id", () => {
  ...
  it("[FRS-6.1] SHALL create a new NoteVersion when isExplicitSave is true even though the previous snapshot (created by POST /notes on the same note) is still well within the 5-minute throttle window", async () => {
    const { accessToken } = await createAuthedUser(
      "explicit-save-update@example.com",
    );
    const createRes = await request(app)
      .post(ROUTES.NOTES_ROOT)
      .set("Authorization", `Bearer ${accessToken}`)
      .send({ title: "Explicit Save Note", body: "v1 body" });
    const noteId = createRes.body.data.id as string;

    const res = await request(app)
      .patch(ROUTES.noteById(noteId))
      .set("Authorization", `Bearer ${accessToken}`)
      .send({ body: "v2 body", isExplicitSave: true });
    ...
    expect(versions[0]?.bodySnapshot).toBe("v2 body");
  });

  it("[FRS-6.1] SHALL update the Note row but create no new NoteVersion when isExplicitSave is false/omitted and the latest snapshot is well within the 5-minute throttle window", async () => {
    const { accessToken } = await createAuthedUser(
      "autosave-within-throttle@example.com",
    );
    const createRes = await request(app)
      .post(ROUTES.NOTES_ROOT)
      .set("Authorization", `Bearer ${accessToken}`)
      .send({ title: "Autosave Note", body: "v1 body" });
    const noteId = createRes.body.data.id as string;

    const res = await request(app)
      .patch(ROUTES.noteById(noteId))
      .set("Authorization", `Bearer ${accessToken}`)
      .send({ body: "v2 body autosave" });

    expect(res.status).toBe(200);
    expect(res.body.data.body).toBe("v2 body autosave");
    const versions = await prisma.noteVersion.findMany({
      where: { noteId },
    });
  ...
  it("[FRS-6.1] SHALL create a new NoteVersion when isExplicitSave is false/omitted and at least 5 minutes have elapsed since the latest snapshot", async () => {
    const { accessToken } = await createAuthedUser(
      "autosave-past-throttle@example.com",
    );
    const createRes = await request(app)
      .post(ROUTES.NOTES_ROOT)
      .set("Authorization", `Bearer ${accessToken}`)
      .send({ title: "Stale Snapshot Note", body: "v1 body" });
    const noteId = createRes.body.data.id as string;
    const throttleMs = APP_LIMITS.VERSION_SNAPSHOT_THROTTLE_MINUTES * 60 * 1000;
    await prisma.noteVersion.updateMany({
      where: { noteId },
      data: { createdAt: new Date(Date.now() - (throttleMs + 1000)) },
    });

    const res = await request(app)
      .patch(ROUTES.noteById(noteId))
      .set("Authorization", `Bearer ${accessToken}`)
      .send({ body: "v2 body past throttle" });
    ...
    expect(versions).toHaveLength(2);
    expect(versions[0]?.bodySnapshot).toBe("v2 body past throttle");
  });
});
